<?php
// Shared trust marquee bar — used on hero sections across all pages
$trust_items = [
...
$all = array_merge($trust_items, $trust_items);
?>
<div class="trust-marquee-outer" style="background:rgba(7,16,30,0.75);backdrop-filter:blur(12px);border-top:1px solid rgba(200,132,90,0.15)">
  <div class="trust-marquee-track">
    <?php foreach ($all as $t): ?>
    <div class="trust-item">
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><?= $t['icon'] ?></svg>
      <?= $t['label'] ?>
    </div>
    <?php endforeach; ?>
  </div>
